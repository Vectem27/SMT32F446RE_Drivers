...
typedef struct __SD_SPI_Handle
{
    SD_SPI_Init init;

    SDCardType card_type;
...
typedef struct _SD_SCR
{
    ...
    /*!< :4 (=0, else reserved) SCR structure version */
    uint8_t SCR_Version;

    /*!< :4 Physical layer specification version number
    ...
    /*!< :1 State of bits after sector erase */
    uint8_t StateAfterErase;

    /*!< :3 CPRM security version
     0 - no security
     1 - not used
    ...
    /*!< :1 Distinguish between SpecVersion 2.00 or 3.0x */
    uint8_t SpecVersion3;

    /*!< :4 Extended security (0 if not supported) */
    uint8_t ExSecurity;

    /*!< :9 Reserved */
    uint16_t Reserved1;

    /*!< :1 Support of CMD23 (set block count) */
    uint8_t CmdSupport1;

    /*!< :1 Support of CMD20 (speed class control) */
    uint8_t CmdSupport2;
...
/**
 * @brief  Init result error
 */
typedef enum _SD_InitResult
{
    SD_INIT_SUCESS = 0x00,
    SD_INIT_NO_CARD = 0x01,
    SD_INIT_VOLTAGE_ERROR = 0x2,
    SD_INIT_APP_INIT_FAILED = 0x03,
    SD_INIT_UNKNOW_CARD = 0x04,
